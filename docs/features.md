...
- **Persistence:** `src/storage/discussionStorage.ts` — `DiscussionStorageProvider` with local-JSON-file (default) and Supabase backends behind `createDiscussionStorage()` / `DB_PROVIDER`; `save` upserts by id and caps at `MAX_DISCUSSIONS_PER_USER`. Types (`StoredDiscussion`, `DiscussionListItem`, `DiscussionPhase`) live in `src/storage/types.ts`; the `discussions` table is in `supabase/schema.sql`. Client glue (`src/app/discuss/sessionStore.ts`) holds the localStorage live-transcript helpers and the history API client.
