...
4. **Authorization seam in storage.** _**[✅ Closed 2026-06-30:** ownership moved into the contract via `getOwned(id, userId)`; raw `get()` is now internal/test-only. See [refactor-opportunities/plan.md](../changes/refactor-opportunities/plan.md).**]**_ As of 06-15: `StorageProvider.get(id)` took **no `userId`** `[code: storage/types.ts:30]`; ownership was enforced only by a manual check in the route. One forgotten check = an IDOR breach. `[code][inference]`
