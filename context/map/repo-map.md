...
2. **Documentation ↔ code divergence (the "Stage 2" gap).** README/PRD advertise a 3-stage flow with **peer review & ranking**; the code implements **only 2 phases** (specialists → judge) and has no peer-review prompt at all. A reviewer following the docs will look for a feature that isn't there. `[code: runCouncil.ts:380-563, buildPrompts.ts:19-36; README.md:23-25]`
