...
export class MockProvider implements LLMProvider {
  async generate(input: GenerateInput): Promise<GenerateOutput> {
    ...
    // AI code-review agent (10xChampion tool): recognise the review sentinel and
    // return a deterministic, schema-valid verdict derived from cheap diff heuristics,
    // so `npm run review` works end-to-end keyless under LLM_PROVIDER=mock.
    if (input.systemPrompt.includes("10X-CODE-REVIEW")) {
      const diff = input.userMessage;
      const added = diff
        .split("\n")
      ...
      content = JSON.stringify({
        ...
        findings: risky
          ? [{ severity: "major", note: "Potentially unsafe dynamic/interpolated pattern in added lines." }]
          : [],
      });
